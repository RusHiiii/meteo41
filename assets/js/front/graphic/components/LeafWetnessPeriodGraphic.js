...
function useLeafWetnessPeriodGraphic({ data, history, period, unit }, inView) {
  const initialState = {
    series: [],
    options: {
      colors: ['#e7bf22'],
  ...
  useEffect(() => {
    ...
    dispatch({
      ...
      maxTime: new Date(history.maxLeafWetnessReceivedAt).getTime(),
      maxValue: history.maxLeafWetness,
    });
  }, [history, inView]);
...
function LeafWetnessPeriodGraphic(props) {
  const { ref, inView, entry } = useInView({ threshold: 0 });
  const [state, dispatch] = useLeafWetnessPeriodGraphic(props, inView);
...
export default React.memo(LeafWetnessPeriodGraphic);
